...
public class ProductPlanServiceImpl implements ProductPlanService {
	...
	public ProductPlan addProductPlan(ProductPlan productplan) {
		...
		if (productplan1!= null) {
			...
			throw new PricingException("Unable to add Productplan or record already exists");
		}
	}
	...
	public ProductPlan updateProductPlan(ProductPlan productplan) {
		...
		if (productplan!= null) {
			...
			throw new PricingException("Unable to update the  Product or no matching record found to update");
		}
	}

	@Override
	public ProductPlan getProductPlan(int planId) {
		ProductPlan productplan1 = productplandao.getProductPlan(planId);
		if (productplan1!= null) {
			return productplan1;
		} else {
			throw new PricingException("Unable to fetch the Productplan details no matching record found to fetch ");
		}
	...
	public ProductPlan deleteProductPlan(int planId) {
		...
		if (productplanDto!= null) {
			...
			throw new PricingException("Unable to detele the propductplan or no matching data found to delete");
		}
	}
